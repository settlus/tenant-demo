...
export interface MissionState {
  mission: number
  handleMission: (mission: number) => void
  reset: () => void
}
...
const MissionProvider: React.FC<GlobalProviderProps> = ({ children }) => {
  const [mission, setMission] = useState<number>(
    parseInt(sessionStorage.getItem('mission') || '0', 10)
  )
  ...
  return <MissionContext.Provider value={state}>{children}</MissionContext.Provider>
}

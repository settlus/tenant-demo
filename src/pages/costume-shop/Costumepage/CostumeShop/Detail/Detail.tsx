...
export default function Detail() {
  ...
  return (
    <CostumeShopContainer
      ...
      widthRatio="flex-[2]"
    >
      <div className="rounded-lg bg-[#F9FAFB] gap-2 w-auto h-full">
        <CostumeShopTitle name="Detail" />
        <div className="flex flex-col p-2 my-2 gap-4">
          <div className="w-[200px] h-[200px] relative">
            <img
              className="h-[20%] w-[20%] absolute top-0 right-0 mr-2 mt-2"
              src={nftIcon}
            />
            <img
              className="h-[70%] w-[70%] mx-auto"
              src={selectedItem.thumbnailPng}
            />
          </div>
          ...
          <div className='pl-1'>
            ...
            />     */}
              ...
              >
                ...
                <br />
                Total sold amount:{" "}
                {formatNum(selectedItem.quantity * selectedItem.price)}
              </span>
            </p>
          </div>
        </div>
        ...
        <div id="sample">
          {selectedItem.userCreated ? (
            <GuidePointer doGuide={true}>
              <button className={styles.dashboardButton} onClick={handleClick}>
                Go to my dashboard
              </button>
            </GuidePointer>
          ) : (
            <button className="w-[200px] h-[50px] p-[13px_28px] gap-[10px] rounded-[6px] bg-[#E5E7EB]">Buy</button>
          )}
        </div>
      </div>
    </CostumeShopContainer>

...
console.log("Emscripten version: 3.1.45");
...
var Module = {
	...
	OVDR_FilePath: '', // this is the path to the .uproject file, determined by external settings
	// OVDR_Thumbnails 객체 정의
	OVDR_Thumbnails : 
	{
		main: {
			file: null,
...
Module.arguments = sessionStorageCommandLine != null ? [sessionStorageCommandLine] : ['../../../OVDRClothWeb/OVDRClothWeb.uproject','-stdout',];
...
document.addEventListener('error', function(){
	if (document.getElementById('clear_indexeddb')) {
...
Module.printErr = function(text) {
	console.error(text);
	if (text.toLowerCase().indexOf('memory') != -1) {
		reportError(text)
	}
};
...
var canvasAspectRatioWidth = 600; // 1366;
var canvasAspectRatioHeight = 800; // 768;
...
function resizeCanvas(aboutToEnterFullscreen) {
	...
	// MEMO: change logic adjust to eterno web
	// var cssHeight = Math.max(minimumCanvasHeightCssPixels, mainAreaRect.bottom - mainAreaRect.top - buttonAreaRect.bottom + buttonAreaRect.top);
	var cssHeight = Math.max(minimumCanvasHeightCssPixels, mainAreaRect.bottom - mainAreaRect.top);
	...
	return new Promise((resolve, reject) => {
	  try {
		if (UE_JSOVDRClothHTML5.RegisterCallbacks) {
		  UE_JSOVDRClothHTML5.RegisterCallbacks('SelectTemplate', () =>
			resolve('SelectTemplate Done'),
		  )
		}
		if (UE_JSOVDRClothHTML5.Callback_RegisterSelectTemplateFunction) {
		  UE_JSOVDRClothHTML5.Callback_RegisterSelectTemplateFunction(templateName)
		}
	  } catch (e) {
		console.error(e)
		reject(e)
...
Module['OVDR_ApplyPNG'] = function (imagePath, templateName) {
	...
	return new Promise(async (resolve, reject) => {
	  try {
		if (UE_JSOVDRClothHTML5.RegisterCallbacks) {
		  UE_JSOVDRClothHTML5.RegisterCallbacks('ApplyPNG', () => resolve('ApplyPNG Done'))
		}
		if (UE_JSOVDRClothHTML5.Callback_RegisterEquipCostumeFunction) {
		  await fetch(imagePath)
			...
			.then((arrayBuffer) => {
			  ...
			  UE_JSOVDRClothHTML5.Callback_RegisterEquipCostumeFunction(
				templateName,
				dataPtr,
				data.length,
			  )
		...
		console.error('Error loading PNG file:', e)
		reject(e)
	  }
	})
}
  
Module['OVDR_Restore'] = function (templateName) {
	console.log('OVDR_RestoreCostume')
	return new Promise((resolve, reject) => {
	  try {
		if (UE_JSOVDRClothHTML5.RegisterCallbacks) {
		  UE_JSOVDRClothHTML5.RegisterCallbacks('RestoreCostume', () =>
			resolve('RestoreCostume Done'),
		  )
		}
		if (UE_JSOVDRClothHTML5.Callback_RegisterRestoreCostumeFunction) {
		  UE_JSOVDRClothHTML5.Callback_RegisterRestoreCostumeFunction(templateName)
		}
	  } catch (e) {
		console.error(e)
		reject(e)
...
Module['OVDR_ResetAvatar'] = function () {
	...
	return new Promise((resolve, reject) => {
	  try {
		if (UE_JSOVDRClothHTML5.RegisterCallbacks) {
		  UE_JSOVDRClothHTML5.RegisterCallbacks('ResetAvatar', () => resolve('ResetAvatar Done'))
		}
		if (UE_JSOVDRClothHTML5.Callback_RegisterRestoreAvatarFunction) {
		  UE_JSOVDRClothHTML5.Callback_RegisterRestoreAvatarFunction()
		}
	  } catch (e) {
		console.error(e)
		reject(e)
...
Module['OVDR_CaptureThumbnail'] = function (templateName) {
	...
	return new Promise((resolve, reject) => {
	  try {
		if (UE_JSOVDRClothHTML5.RegisterCallbacks) {
		  UE_JSOVDRClothHTML5.RegisterCallbacks('CaptureThumbnail', () =>
			resolve('CaptureThumbnail Done'),
		  )
		}
		if (UE_JSOVDRClothHTML5.Callback_RegisterThumbnailFunction) {
		  UE_JSOVDRClothHTML5.Callback_RegisterThumbnailFunction(templateName)
		}
	  } catch (e) {
		console.error(e)
		reject(e)
...
Module['OVDR_ApplyCustomize'] = function (CustomizeType) {
	...
	return new Promise((resolve, reject) => {
	  try {
		if (UE_JSOVDRClothHTML5.RegisterCallbacks) {
		  UE_JSOVDRClothHTML5.RegisterCallbacks('ApplyCustomize', () =>
			resolve('ApplyCustomize Done'),
		  )
		}
		if (UE_JSOVDRClothHTML5.Callback_RegisterApplyCustomizeFunction) {
		  UE_JSOVDRClothHTML5.Callback_RegisterApplyCustomizeFunction(CustomizeType)
		}
	  } catch (e) {
		console.error(e)
		reject(e)
...
Module.locateFile = function(name) {
	...
	name = Module.OVDR_FilePath + name
...
function reportError(errorText = "") {
	// call callback function for error reporting
...
Module['init'] = () => new Promise((initResolve, initReject) => { 
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('OVDRClothWeb-HTML5-Shipping.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('OVDRClothWeb-HTML5-Shipping.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('OVDRClothWeb-HTML5-Shipping.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('OVDRClothWeb-HTML5-Shipping.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('OVDRClothWeb-HTML5-Shipping.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('OVDRClothWeb-HTML5-Shipping.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
